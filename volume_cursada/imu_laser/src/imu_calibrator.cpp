...
void robmovil_ekf::IMUCalibrator::calculate_bias(){
  ...
  for (const auto &v : calibration_data_) {
    bias_.setX(bias_.getX() + v.getX());
    bias_.setY(bias_.getY() + v.getY());
    bias_.setZ(bias_.getZ() + v.getZ());
  }

  const double n = static_cast<double>(calibration_data_.size());

  bias_.setX(bias_.getX() / n);
  bias_.setY(bias_.getY() / n);
  bias_.setZ(bias_.getZ() / n);
...
void robmovil_ekf::IMUCalibrator::on_imu_measurement(const sensor_msgs::msg::Imu& msg)
{
  ...
  if ( is_calibrating_ ) {
    ...
    tf2::Vector3 delta_theta = delta_t * vel_sin_bias;

    // Convertir a quaternion
    tf2::Quaternion delta_q;
    delta_q.setRPY(0, 0, delta_theta.getZ());
