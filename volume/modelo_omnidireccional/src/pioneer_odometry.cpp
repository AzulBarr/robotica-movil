...
void PioneerOdometry::on_velocity_cmd(const geometry_msgs::msg::Twist::SharedPtr twist)
{
  ...
  /*
  double vFrontLeft = (angVFrontLeft * WHEEL_RADIUS) / cos(45);
  double vFrontRight = (angVFrontRight * WHEEL_RADIUS) / cos(45);
  double vRearLeft = (angVRearLeft * WHEEL_RADIUS) / cos(45); //ecuacion 1, decia mirar desde la 19 a la 26
  double vRearRight = (angVRearRight * WHEEL_RADIUS) / cos(45);*/
...
void PioneerOdometry::on_encoder_ticks(const robmovil_msgs::msg::MultiEncoderTicks::SharedPtr encoder)
{
  ...
  if (!ticks_initialized_) {
    ...
    last_ticks_front_left_ = encoder->ticks.data[0]; //revisar
    last_ticks_front_right_ = encoder->ticks.data[1];
    last_ticks_rear_left_ = encoder->ticks.data[2]; 
    last_ticks_rear_right_ = encoder->ticks.data[3];
    last_ticks_time = encoder->header.stamp;
    return;
  }

  int32_t delta_ticks_front_left_ = encoder->ticks.data[0] - last_ticks_front_left_;
  int32_t delta_ticks_front_right_ = encoder->ticks.data[1] - last_ticks_front_right_;
  int32_t delta_ticks_rear_left_ = encoder->ticks.data[2] - last_ticks_rear_left_;
  int32_t delta_ticks_rear_right_ = encoder->ticks.data[3] - last_ticks_rear_right_;
