...
void robmovil_ekf::LocalizerEKF::makeA(void)
{
  ...
  A(1,3) = 0;//-sin(theta_0) * v0 * delta_t;
  A(2,3) = 0;//cos(theta_0) * v0 * delta_t;
...
void robmovil_ekf::LocalizerEKF::makeProcess(void)
{
  ...
  double theta = x_old(3);
  double vx = u(1);
  double vy = u(3);
  double omega = u(2);

  x(1) = x_old(1) + (vx * cos(theta) - vy * sin(theta)) * delta_t;
  x(2) = x_old(2) + (vx * sin(theta) + vy * cos(theta)) * delta_t;
  x(3) = angles::normalize_angle(theta + omega * delta_t);
  ...
  //  " !!!! Process model: X_t-1 = (%f, %f, %f), X_t = (%f, %f, %f), delta_t = %f",
...
bool robmovil_ekf::LocalizerEKF::find_corresponding_landmark(const tf2::Vector3& measured_landmark, tf2::Vector3& corresponding_landmark, float delta_radio)
{
  ...
  {
    /* COMPLETAR */
    float distancia = sqrt( pow( map_landmarks[i].getX() - measured_landmark.getX(), 2 ) +
                           pow( map_landmarks[i].getY() - measured_landmark.getY(), 2 ) ) ;
                          // pow( map_landmarks[i].getZ() - measured_landmark.getZ(), 2 ) ) ;    
    
    if (distancia < min_distance && distancia <= delta_radio) {
...
robmovil_ekf::LocalizerEKF::Vector robmovil_ekf::LocalizerEKF::landmark2measure(const tf2::Vector3& landmark)
{
  ...
  //if (relative_landmark.getX() != 0 || relative_landmark.getY() != 0) {
