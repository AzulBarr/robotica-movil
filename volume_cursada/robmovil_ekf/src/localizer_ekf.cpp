...
void robmovil_ekf::LocalizerEKF::makeA(void)
{
  ...
  A(1,3) = -sin(theta_0) * v0 * delta_t;
  A(2,3) = cos(theta_0) * v0 * delta_t;
...
void robmovil_ekf::LocalizerEKF::makeProcess(void)
{
  ...
  double theta_0 = x_old(3);
  double v0 = u(1);
   
  x(1) = x_old(1) + v0 * cos(theta_0) * delta_t;
  x(2) = x_old(2) + v0 * sin(theta_0) * delta_t;
  x(3) = angles::normalize_angle(theta_0 + u(2) * delta_t);
